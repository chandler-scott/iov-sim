...
std::unordered_map<std::string, PyObject*> AggregatorWrapper::getStateDicts()
{
    std::unordered_map<std::string, PyObject*> stateDicts;

    if (pAggregator) {
        // Get the policy attribute and its p_net attribute (NeuralNetwork)
        PyObject* pPolicy = PyObject_GetAttrString(pAggregator, "policy");
        PyObject* pPNet = PyObject_GetAttrString(pPolicy, "p_net");
        Py_DECREF(pPolicy);

        // Get the value attribute and its v_net attribute (NeuralNetwork)
        PyObject* pValue = PyObject_GetAttrString(pAggregator, "value");
        PyObject* pVNet = PyObject_GetAttrString(pValue, "v_net");
        Py_DECREF(pValue);

        // Use the state_dict() method to get the state dictionaries of the neural networks
        PyObject* pPNetStateDict = PyObject_CallMethod(pPNet, "state_dict", nullptr);
        PyObject* pVNetStateDict = PyObject_CallMethod(pVNet, "state_dict", nullptr);

        if (pPNetStateDict && pVNetStateDict) {
            stateDicts["p_net"] = pPNetStateDict;
            stateDicts["v_net"] = pVNetStateDict;
        ...
        // No need to release pAggregator, pPNet, or pVNet since they're already provided from outside this function
    }
    else {
        PyErr_Print();
    }
