...
void BaseApplicationLayer::initialize(int stage)
{
    ...
    if (stage == 0) {
        ...
        if (par("avoidBeaconSynchronization").boolValue() == true) {
            ...
            if (sendBeacons) {
                std::cout << myId << ": sending beacon" << std::endl;
...
void BaseApplicationLayer::receiveSignal(cComponent* source, simsignal_t signalID, cObject* obj, cObject* details)
{
    Enter_Method_Silent();
...
void BaseApplicationLayer::handlePositionUpdate(cObject* obj)
{
...
void BaseApplicationLayer::handleParkingUpdate(cObject* obj)
{
...
void BaseApplicationLayer::handleSelfMsg(cMessage* msg)
{
    ...
    case SEND_WSA_EVT: {
        std::cout << myId << " BaseApplicationLayer: SEND_WSA_EVT" << std::endl;
...
void BaseApplicationLayer::startService(Channel channel, int serviceId, std::string serviceDescription)
{
...
void BaseApplicationLayer::stopService()
{
    cancelEvent(sendWSAEvt);
    currentOfferedServiceId = -1;
}

void BaseApplicationLayer::sendDown(cMessage* msg)
{
    checkAndTrackPacket(msg);
    BaseApplLayer::sendDown(msg);
}

void BaseApplicationLayer::sendDelayedDown(cMessage* msg, simtime_t delay)
{
    checkAndTrackPacket(msg);
    BaseApplLayer::sendDelayedDown(msg, delay);
}

void BaseApplicationLayer::checkAndTrackPacket(cMessage* msg)
{
